...
def run_json_stage(
    ...
    for candidate_model in models:
        try:
            raw = gemini_generate(
                api_key, candidate_model, prompt, temperature, max_tokens
            )
            return parse_json(raw)
        except Exception as exc:
            last_error = exc

    raise WorkflowError(f"{stage_name} failed: {last_error}")
...
def run_study_workflow(
    ...
    try:
        ...
        result["review"] = {
            "status": "skipped",
            "overall_score": None,
            "feedback": "Assignment review was temporarily unavailable.",
            "error": str(exc),
        }
